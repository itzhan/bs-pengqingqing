...
axiosInstance.interceptors.response.use(
  (response: AxiosResponse<BaseResponse>) => {
    ...
    // 兼容后端 message 字段映射为 msg
    if (!data.msg && (data as any).message) {
      data.msg = (data as any).message
    }
    const { code, msg } = data
  ...
  (error) => {
    ...
    const isAuthInfoRequest = requestUrl.includes('/api/auth/info')

    if (
      status === ApiStatus.unauthorized ||
      (status === ApiStatus.forbidden && isAuthInfoRequest)
    ) {
